import React from 'react'

const Navbar = () => {
  return (
    <>
<div className='bg-black'>
    <nav className='w-full h-14 bg-indigo-200 flex justify-between px-4 md:px-4 items-center'>
      <div className='text-indigo-700 text-2xl font-bold'>NIT Nagaland with PW</div>
        <ul className='md:flex hidden font-semibold'>
          <li className='mx-[10px] cursor-pointer hover:text-white'>Home</li>
          <li className='mx-[10px] cursor-pointer hover:text-white'>About Us</li>
          <li className='mx-[10px] cursor-pointer hover:text-white'>Contact Us</li>
        </ul>
        <div className='hidden md:block px-2 py-2 bg-indigo-700 text-white rounded font-bold cursor-pointer hover:text-black'>Login/SignUp</div>
        <div className='md:hidden'>
            <a className='text-4xl' href="#">&#8801;</a>
        </div>
    </nav>
</div>
    
    </>
  )
}

export default Navbar